...
static int virtblk_get_info(int fd, __u64 *nlba){

	int err;

	err = ioctl(fd, BLKGETSIZE64, nlba);
	printf("nlba is 0x%llx \n",*nlba);
        if (err < 0)
                return err;
	return 0;
}
...
static int get_file_size(struct file *f)
{
	...
	if (pt) {
		...
		if (!S_ISCHR(st.st_mode)) {
			fprintf(stderr, "passthrough works with only vblk "
					"generic devices (/dev/vdXc0)\n");
			return -1;
		}
//		ret = virtblk_get_info(f->real_fd, &nlba);
//		if (ret)
//			return -1;
//		if ((bs % lbs) != 0) {
...
//			return -1;
//		}
		f->max_blocks = 10995116277760 / 4096;
		f->max_size = 10995116277760;
...
static int submitter_init(struct submitter *s)
{
	...
	if (pt) {
		for (i = 0; i < roundup_pow2(depth); i++) {
			...
			memset(&sqe->cmd, 0, sizeof(struct virtblk_uring_cmd));
		}
	}
	return nr_batch;
}
...
static void usage(char *argv, int status)
{
	...
	printf("%s [options] -- [filenames]\n"
		...
		" -u <bool> : Use virtblk-passthrough I/O, default %d\n",
